...
export const KPICards: React.FC<KPICardsProps> = ({
    ...
    const getCategoryIcon = (cat: string) => {
        switch (cat) {
            case 'Maquinaria': return <Box size={16} className="text-blue-500" />;
            case 'Redes': return <Share2 size={16} className="text-emerald-500" />;
            case 'Infra': return <Layers size={16} className="text-indigo-500" />;
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-100/50 p-6 rounded-3xl border border-slate-200/60 shadow-inner">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h2 className="text-sm font-black text-slate-800 uppercase tracking-widest">Ejecución por Clasificación</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Maquinaria, Redes e Infraestructura</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {categorySummaries.sort((a, b) => b.totalBudget - a.totalBudget).map((cat) => (
                        <div key={cat.category} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:translate-y-[-2px] transition-transform">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex justify-between items-end">
                                        <span className="text-sm font-black text-slate-800">{formatCurrency(cat.totalBudget)}</span>
                                        <span className={`text-xs font-bold ${cat.totalReal > cat.totalBudget ? 'text-pf-red' : 'text-slate-600'}`}>
                                ...
                                <div className="space-y-1">
                                    <div className="flex justify-between text-[10px]">
                                        <span className="text-slate-400 font-bold">DESVIACIÓN</span>
                                        <span className={`font-black ${cat.totalReal > cat.totalBudget ? 'text-pf-red' : 'text-emerald-500'}`}>
                                            {cat.deviation.toFixed(1)}%
                                        </span>
                                    </div>
                                    <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden border border-slate-300/30">
                                        <div
                                            className={`h-full rounded-full transition-all duration-700 ${cat.totalReal > cat.totalBudget ? 'bg-pf-red' : 'bg-emerald-500'}`}
                                            style={{ width: `${Math.min((cat.totalReal / (cat.totalBudget || 1)) * 100, 100)}%` }}
                                        />
                                    </div>
                                </div>

                                <div className="pt-2 border-t border-slate-50 flex justify-between items-center text-[9px] text-slate-400">
                                    <span>{cat.itemCount} Activos</span>
                                    <span>{cat.category === 'Maquinaria' ? 'Equipos PF' : cat.category === 'Redes' ? 'Edificios' : 'Infra'}</span>
                                </div>
                            </div>
                        </div>
                    ))}
